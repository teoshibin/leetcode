...
def run_solution(problem_number, solution_number=None):
    ...
    try:
        ...
        solution_classes = [cls for cls in module.__dict__.values() if isinstance(
            cls, type) and cls.__module__ == module.__name__]

        if solution_number is not None:
            solution_classes = [cls for cls in solution_classes if cls.__name__.endswith(
                f"Solution{solution_number}")]

        for solution_class in solution_classes:
            solution = solution_class()
            solution.run()  # This is the new preserved 'run' method
